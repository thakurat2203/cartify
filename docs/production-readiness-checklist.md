...
`MONGO_URI`, `JWT_ACCESS_SECRET`, `JWT_REFRESH_SECRET`, and production `CLIENT_URL` are required by `server/src/config/index.js`.
...
- Use long, random, different `JWT_ACCESS_SECRET` and `JWT_REFRESH_SECRET` values.
- Keep both secrets only in backend environment variables.
- Never expose either secret to the frontend.
- Rotate a secret if it leaks.
- Understand that rotating either secret invalidates its corresponding tokens.

Recommended:

```text
Use at least 32 random characters for each JWT secret.
```
